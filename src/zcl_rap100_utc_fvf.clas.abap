"! @testing BDEF:ZR_TRAP100_FVF
CLASS zcl_rap100_utc_fvf DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC
  FOR TESTING
...
CLASS ZCL_RAP100_UTC_FVF IMPLEMENTATION.


  METHOD class_setup.
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                         i_for_entities = VALUE #(
                           ( i_for_entity = 'ZR_TRAP100_FVF' ) ) ).
...
*            FROM VALUE #( ( %cid_ref = 'ROOT1' ) )
     ...
     cl_abap_unit_assert=>assert_not_initial( msg = 'mapped-travel'  act = mapped-ZrTrap100Fvf ).
    ...
    ENDIF.
     ...
     SELECT * FROM ZR_TRAP100_FVF INTO TABLE @DATA(lt_travel). "#EC CI_NOWHERE
